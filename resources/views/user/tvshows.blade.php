    ...
    <header id='header'>
        <nav class="navbar text-black">
            ...
            <ul class="nav-menu mt-1">
                ...
                <li class="nav-item">
                    <button class="btn  mt-2 ps-2 pe-2 me-0 search text-light" onclick="myFunction()" id='button'><i class="fa-solid fa-magnifying-glass"></i></button>
                </li>
        ...
        <div class="card-body">
            <div class="line">
                <div class="movierow ">
                    ...
                    <a class="column" style="position: relative;" href="{{route('user#tvshowDetails',$item->tvshows_id)}}">
                        <div class="moviecard" style="width: 200px; height:350px; overflow:hidden" onmouseover="myOpacity('{{$item->tvshows_id}}')" onmouseleave="myNoOpacity('{{$item->tvshows_id}}')">
                            <img src="{{asset('uploads/'.$item->poster)}}" alt=" " width="200px " height="300px " class="image ">
                            <p class="text-center p-1 ">{{$item->tvshows_name}}</p>
                        </div>
                        <div class="overlay text-center" id="overlay{{$item->tvshows_id}}" style="opacity: 0;">
                            <h5>{{$item->tvshows_name}}</h5>
...
function buttonClick2() {
    ...
    return;
            ...
            return datas.tvshows_name.toLowerCase().includes(search);
            })
        //  console.log(filteredProduct[0].tvshows_id.toString());
        ...
        if(hasProductToShow){
...

            {
                ...
                productItemContainer.id = filteredProduct[i].tvshows_id;
                productItemContainer.classList.add('productItemContainer');

                const productName = document.createElement('div');
                productName.classList.add('productName');
                productName.append(filteredProduct[i].tvshows_name);
...
const selectProduct = (index) => {
  const productIdToSelect = filteredProduct[index].tvshows_id.toString();
